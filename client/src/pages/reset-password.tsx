...
export default function ResetPasswordPage() {
  ...
  console.log('[Reset Password] Full location:', location);
  ...
  // Use window.location.search to get query parameters properly
  const urlParams = new URLSearchParams(window.location.search);
  const token = urlParams.get('token');
  console.log('[Reset Password] Parsed token:', token ? token.substring(0, 10) + '...' : 'null/undefined');
  ...
  useEffect(() => {
    if (!token) {
      console.log('[Reset Password] No token found, redirecting to login');
      toast({
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    if (!token) {
      console.log('[Reset Password] No token available for submission');
      return;
    }

    console.log('[Reset Password] Submitting reset request with token:', token.substring(0, 10) + '...');
    console.log('[Reset Password] New password provided');
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('[Reset Password] Reset failed with error:', errorData);

        // Check if it's a login credentials issue
        if (errorData.message && errorData.message.includes('Invalid or expired reset token')) {
          toast({
            title: "Reset Failed",
            description: "The reset link has expired or is invalid. Please request a new password reset.",
            variant: "destructive",
          });
        } else {
          toast({
      ...
      console.log('[Reset Password] Network error:', error);
      toast({
        title: "Network Error",
